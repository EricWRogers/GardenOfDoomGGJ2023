...
{    
    ...
    highp float alpha = min(color.a, texture(material.diffuse, TexCoords).a);
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    highp float attenuation = 1.0 / (light.constant + light.linear * distance + light.quadratic * (distance * distance));    
    // combine results
    vec3 ambient = light.ambient * (vec3(color) * vec3(texture(material.diffuse, TexCoords)));
    vec3 diffuse = light.diffuse * diff * (vec3(color) * vec3(texture(material.diffuse, TexCoords)));
    vec3 specular = light.specular * spec * vec3(texture(material.specular, TexCoords));
